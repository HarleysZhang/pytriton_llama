...
class FusedAttention(nn.Module):
    def __init__(self,  args: ModelArgs):
        ...
        self.q_proj_weight = nn.Parameter(torch.rand(self.hidden_size, self.hidden_size))
        self.k_proj_weight = nn.Parameter(torch.rand(self.hidden_size, self.hidden_size))
        self.v_proj_weight = nn.Parameter(torch.rand(self.hidden_size, self.hidden_size))
        self.o_proj_weight = nn.Parameter(torch.rand(self.hidden_size, self.hidden_size))
    ...
    def forward(self, 
        ...
        # (B, 1, H_Q, Head_Dim) --> (B, 1, H_Q, Head_Dim)
        xq = rope(xq, freqs_complex, device=x.device)
        # (B, 1, H_KV, Head_Dim) --> (B, 1, H_KV, Head_Dim)
        xk = rope(xk, freqs_complex, device=x.device)
...
class FusedMLP(nn.Module):
    def __init__(self, args: ModelArgs):
        ...
        self.gate_proj_weight = nn.Parameter(torch.rand(args.dim, hidden_dim))
        self.up_proj_weight = nn.Parameter(torch.rand(hidden_dim, args.dim))
        self.down_proj_weight = nn.Parameter(torch.rand(args.dim, hidden_dim))
...
class LlamaDecoderLayer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, start_pos: int, freqs_complex: torch.Tensor):
        ...
        h = x + self.attention.forward(
           post_attention, start_pos, freqs_complex
        )
...
class Llama(nn.Module):
    ...
    def __init__(self, args: ModelArgs):
        ...
        for layer_id in range(args.n_layers):
            self.layers.append(LlamaDecoderLayer(args))
